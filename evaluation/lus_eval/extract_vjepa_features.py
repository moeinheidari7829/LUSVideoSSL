...
# V-JEPA is an external dependency.
# Set VJEPA_REPO to the path of the V-JEPA repository before running.
vjepa_repo = os.environ.get("VJEPA_REPO")

if not vjepa_repo:
    raise RuntimeError(
        "VJEPA_REPO is not set. "
        "Set it to the path of the V-JEPA repository."
    )

VJEPA_REPO = Path(vjepa_repo).expanduser().resolve()

if not VJEPA_REPO.is_dir():
    raise FileNotFoundError(
        f"V-JEPA repository not found: {VJEPA_REPO}"
    )
...
def main():
    ...
    def forward_tokens(clips):
        ...
        if isinstance(tokens, list):
            if len(tokens) != 1:
                raise RuntimeError(
                    f"unexpected encoder list length: {len(tokens)}"
                )
            tokens = tokens[0]
        return tokens
